...
tests = do
      TU.suite "subquery" do
            TU.test "null" do
                  ...
                  TM.parameterized """SELECT (SELECT id "id" FROM users WHERE id = $1) FROM messages ORDER BY id LIMIT 1""" $ Query.query q
            ...
            --       TM.notParameterized """SELECT id "id", (SELECT id "id" FROM users WHERE id <> u.id) AS "n" FROM users u""" $ Query.query q
