...
main = TUM.runTest do
      TU.suite "select" do
            ...
            TU.suite "sub query" do
                  TU.test "scalars" do
                        let query = print $ select (3 /\ (select 34 # from users # wher (name .=. surname) {}))
      ...
      TU.suite "where" do
            ...
            TU.suite "parameters" do
                  TU.test "equals" do
                        let query = print $ select id # from users # wher (name .=. (Parameter :: Parameter "name")) {name : "josh"}
                        withParameters "SELECT id FROM users WHERE name = @name" query
                  TU.test "not equals" do
                        let query = print $ select id # from users # wher (id .<>. (Parameter :: Parameter "id")) {id: 3}
                        withParameters "SELECT id FROM users WHERE id <> @id" query

            TU.suite "logical operands" do
                  TU.suite "and" do
                        TU.test "single" do
                              let query = print $ select id # from users # wher (name .=. (Parameter :: Parameter "name") .&&. name .=. surname) {name : "josh"}
                              withParameters "SELECT id FROM users WHERE (name = @name AND name = surname)" query
                        TU.test "many" do
                              let query = print $ select id # from users # wher (name .=. (Parameter :: Parameter "name") .&&. name .=. surname .&&. surname .<>. (Parameter :: Parameter "surname")) {name : "josh", surname: "j."}
                  ...
                  TU.suite "mixed" do
                        TU.test "not bracketed" do
                              let query = print $ select id # from users # wher (id .=. (Parameter :: Parameter "id3") .||. id .=. (Parameter :: Parameter "id33") .&&. id .=. (Parameter :: Parameter "id333")) {id3 : 3, id33: 33, id333 : 333}
