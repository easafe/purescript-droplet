...
tests =
      TU.suite "offset" do
            TU.test "from" do
                  ...
                  TM.notParameterized """SELECT 4 AS "n" FROM users ORDER BY "n" OFFSET 1""" $ Query.query q
                  TM.result q [ { n: 4 } ]
            TU.test "where" do
                  let q = select id # from users # wher (id .<>. 10) # orderBy (id /\ name) # offset 2
                  TM.parameterized """SELECT "id" FROM users WHERE "id" <> $1 ORDER BY "id", "name" OFFSET 2""" $ Query.query q
                  TM.result q []
            TU.test "limit" do
                  let q = select (4 # as n) # from users # orderBy n # limit 5 # offset 4
                  TM.notParameterized """SELECT 4 AS "n" FROM users ORDER BY "n" LIMIT 5 OFFSET 4""" $ Query.query q
                  TM.result q []