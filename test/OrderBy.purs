...
tests =
      TU.suite "order by" do
            ...
            TU.test "field name" do
                  let q = select id # from users # wher (id .=. 1) # orderBy (id /\ name)
                  TM.notParameterized "SELECT id FROM users WHERE id = $1 ORDER BY id, name" $ Query.query q
                  TM.result q [{id: 1}, {id : 2}]
            TU.test "asc" do
                  let q = select (id # as n) # from users # wher (id .<>. 4 ) # orderBy (n /\ (name # asc))
                  TM.notParameterized "SELECT id AS n FROM users WHERE id <> $1 ORDER BY n, name ASC" $ Query.query q
                  TM.result q [{n: 1}, {n: 2}]
            TU.test "desc" do
