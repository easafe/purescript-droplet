...
tests =
      TU.suite "order by" do
            ...
            TU.test "field name" do
                  ...
                  TM.parameterized """SELECT id "id" FROM users WHERE id <> $1 ORDER BY id, name""" $ Query.query q
                  TM.result q [{id: 1}, {id : 2}]
            TU.test "asc" do
                  let q = select (id # as n) # from users # wher (id .<>. 4 ) # orderBy (n /\ (name # asc))
                  TM.parameterized """SELECT id "id" AS "n" FROM users WHERE id <> $1 ORDER BY n, name ASC""" $ Query.query q
                  TM.result q [{n: 1}, {n: 2}]
            TU.test "desc" do
                  let q = select id # from users # orderBy (id # desc)
                  TM.notParameterized """SELECT id "id" FROM users ORDER BY id DESC""" $ Query.query q
