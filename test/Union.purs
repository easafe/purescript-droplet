...
tests = do
      TU.suite "union" do
            TU.test "select" do
                  ...
                  TM.parameterized """(SELECT id FROM users WHERE name = $1 UNION SELECT id FROM users WHERE name <> $2)""" $ Query.query q
                  TM.result q [ { id: 2 }, { id: 1 } ]
            TU.test "as" do
                  let q = (select id # from (users # as b)) `union` (select id # from users # wher (name .<>. "mary"))
                  TM.parameterized """(SELECT id FROM users AS "b" UNION SELECT id FROM users WHERE name <> $1)""" $ Query.query q
                  TM.result q [ { id: 2 }, { id: 1 } ]
            TU.suite "from union" do
                  TU.test "left" do
                        let q = ((select id # from users) `union` (select id # from messages)) `union` (select id # from users)
                        TM.notParameterized """((SELECT id FROM users UNION SELECT id FROM messages) UNION SELECT id FROM users)""" $ Query.query q
                        TM.result q [ { id: 2 }, { id: 1 } ]
                  TU.test "right" do
                        let q = (select id # from users) `union` ((select id # from messages) `union` (select id # from users))
                        TM.notParameterized """(SELECT id FROM users UNION (SELECT id FROM messages UNION SELECT id FROM users))""" $ Query.query q
      ...
      TU.suite "union all" do
            TU.test "select" do
                  ...
                  TM.parameterized """(SELECT id FROM users WHERE name = $1 UNION ALL SELECT id FROM users WHERE name <> $2)""" $ Query.query q
                  TM.result q [ { id: 2 }, { id: 1 } ]
            TU.test "as" do
                  let q = (select id # from (users # as b)) `unionAll` (select id # from users # wher (name .<>. "mary"))
                  TM.parameterized """(SELECT id FROM users AS "b" UNION ALL SELECT id FROM users WHERE name <> $1)""" $ Query.query q
            ...
            TU.suite "from unionAll" do
                  TU.test "left" do
                        ...
                        TM.notParameterized """((SELECT id FROM users UNION ALL SELECT id FROM messages) UNION ALL SELECT id FROM users)""" $ Query.query q
